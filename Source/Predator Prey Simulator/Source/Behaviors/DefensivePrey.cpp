...
void DefensivePrey::update(Prey* owner, const list<Predator*>& movedPredators, const list<Predator*>& stateChangedPredators)
{
    ...
    {
        if (owner->getWeight() * 2 < nearestPredator->getWeight())
        {
            owner->setHeading(utils->angleBetween(position, nearestPosition));
            owner->setBehaviorState(CAUTIOUS);
        }
        else
        {
            owner->setHeading(-utils->angleBetween(position, nearestPosition));
