...
void FleeingPredator::update(Predator* owner, const list<Prey*>& movedPrey, const list<Prey*>& stateChangedPrey)
{
    ...
    if (optimalPrey != NULL)
    {
        if (optimalPrey->getCurrentBehaviorState() != CAUTIOUS || owner->getHungerFactor() > 1.4)
        {
    ...
    else
    {
        ...
        int rand = utils->getRandomInt(0, 2000);
